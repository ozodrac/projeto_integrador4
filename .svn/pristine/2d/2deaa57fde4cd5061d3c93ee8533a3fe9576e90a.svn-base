...
public class CSSMin {
	...
	/**
	 * Main entry point for CSSMin from the command-line. <b>Usage:</b> CSSMin <i>[Input file]</i>,
	 * <i>[Output file]</i>, <i>[DEBUG]</i>
	 * 
	 * @param args
	 *            The command-line arguments
	 */
	public static void run(File inputfile, FileOutputStream output) {
		// if (args.length < 1) {
	 ...
	 */
	public static void formatFile(File inputfile, OutputStream out) {
		try {
			formatFile(new FileReader(inputfile), out);
...
class Selector {
	...
	public Selector(String selector) throws Exception {
		String[] parts = selector.split("\\{"); // We have to escape the { with a \ for the regex,
												// which itself requires escaping for the string.
												// Sigh.
		if (parts.length < 2) {
		...
		if (parts.length > 2) {
			...
			if (contents.charAt(contents.length() - 1) != '}') { // Ensure that we have a leading
																	// and trailing brace.
...
class Constants {
	static final String[] htmlColourNames = { "aliceblue", "antiquewhite", "aqua", "aquamarine",
			...
			"lightcoral", "lightcyan", "lightgoldenrodyellow", "lightgrey", "lightgreen", "lightpink",
