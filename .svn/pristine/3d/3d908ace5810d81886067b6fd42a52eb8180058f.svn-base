...
<div id="wrapper">
	...
	<form id="formulario"action="<%= MapConfig.getInstance().get("CONFIG-DIR_ROOT", "CONFIG-DIR_USER")+"/" %>" method="post" class="formee">
		<input type="hidden" name="uso_id" value="<%= user.getId() %>" />
	
		<div id="toolbar">
			<h1><a href="<%= MapConfig.getInstance().get("CONFIG-DIR_ROOT", "CONFIG-DIR_USER")+"/" %>"><%= bundlePage.getString("label_usuario") %></a> &gt;&gt; <%= bundlePage.getString("label_editar") %></h1>
			<ul id="buttons">
		...
		<div id="content">
			...
			<div class="grid-6-12 clear">
				...
				<input type="text" name="uso_nome" value="<%= user.getNome() %>" class="required" />
			</div>
			<div class="grid-5-12">
				<label><%= bundlePage.getString("email") %> <em class="formee-req">*</em></label>
				<input type="text" name="uso_email" value="<%= user.getEmail() %>" class="required email" />
			</div>
			
			<div class="grid-4-12 clear">
				<label><%= bundlePage.getString("login") %> <em class="formee-req">*</em></label>
				<input type="text" name="uso_login" value="<%= user.getLogin() %>" class="required" />
			</div>
			<div class="grid-3-12">
			...
			<div class="grid-3-12 clear">
				...
				<select name="lna_id" class="required">
					...
					for (EnumLingua lingua : Alingua) {
						String selected = lingua.getId() == user.getLingua().getId() ? "selected='selected'" : "";
						out.print("<option value='"+ lingua.getId() +"' "+ selected +">"+ lingua.getNomeExibicao() +"</option>");
					}
					%>
				</select>
			</div>
			<div class="grid-3-12">
				...
				<select name="pro_id" class="required">
					...
					for (PerfilUsuario perfil : listPerfilUsuario) {
						String selected = perfil.getId() == user.getPerfil().getId() ? "selected='selected'" : "";
						out.print("<option value='"+ perfil.getId() +"' "+ selected +">"+ perfil.getNome() +"</option>");
					}
					%>
				</select>
			</div>
			<div class="grid-3-12">
				...
				<input type="text" name="uso_cpf" value="<%= user.getCpf() %>" class="cpf required" />
			</div>
		</div>
